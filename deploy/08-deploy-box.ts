...
const deployBox: DeployFunction = async function (
  ...
  const simpleGovernance = await ethers.getContractAt("OrganizationGovernance", organizationGovernanceAddress);
  const timeLock = await ethers.getContractAt("TimeLock", timeLockAddress);
  log("Deploying Box...");
  const BoxFactory = await ethers.getContractFactory("Box");
  const box = await upgrades.deployProxy(BoxFactory, [timeLock.address], { initializer: "initialize" });
  await box.deployed();
